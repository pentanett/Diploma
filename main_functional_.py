...
from base_info import my_token, side_profile_token


from my_database import save_vk_user, look_for_pairs_in_database, add_to_database
...
def pairs_selection(user_id):
    ...
    if picked_pairs is not None and len(picked_pairs) > 0:
        ...
        add_to_databse(user_id, picked_pairs_id)
                ...
                if start_dialogue_func(msg_text):
                    ...
                    if connected_pair is None:
                        ...
                        for photo in connected_pair_image:
                            image_message(event.user_id, "Фото", photo.get("photo_id"))
                        message_send(event.user_id,

                                 f"Чтобы показать следующую анкету введите "
                                 f"{next_searching}")
                else:
                    message_send(event.user_id, f"Привет, для поиска необходимо написать {start_dialogue_command}")
